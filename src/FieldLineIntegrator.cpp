...
void FieldLineIntegrator::rk4Step(const XZPoint &start, int yStart, int region,
                                  int direction, XZPoint &end) const
{
  ...
  model_.evaluateStage(p3, yStart, region, direction, 2, k4);
          ...
          direction * h6 * (k1.dxdy + 2.0 * k2.dxdy + 2.0 * k3.dxdy + k4.dxdy);
  end.z = start.z +
          direction * h6 * (k1.dzdy + 2.0 * k2.dzdy + 2.0 * k3.dzdy + k4.dzdy);
}

TraceStatus FieldLineIntegrator::traceLine(const Point3D &seedInd,
                                           std::size_t seedIndex,
                                           const TraceOutputViews &outputs,
                                           int &stateCount, int &trajCount,
                                           int &punctureCount, int &endRegion,
                                           ...
                                           double &iline) const
{
  ...
  const std::size_t stateBase =
      seedIndex * static_cast<std::size_t>(maxStatesPerSeed_);
  const std::size_t trajBase =
      seedIndex * static_cast<std::size_t>(maxTrajPerSeed_);
  const std::size_t punctureBase =
      seedIndex * static_cast<std::size_t>(maxPuncPerSeed_);
  ...
  if (stateBase + static_cast<std::size_t>(maxStatesPerSeed_) >
          outputs.statesSize ||
      trajBase + static_cast<std::size_t>(maxTrajPerSeed_) >
          outputs.trajectoriesSize ||
      punctureBase + static_cast<std::size_t>(maxPuncPerSeed_) >
  ...
  if (punctureValid != nullptr)
  {
    if (punctureBase + static_cast<std::size_t>(maxPuncPerSeed_) >
    ...
    std::fill(punctureValid + punctureBase,
              punctureValid + punctureBase +
                  static_cast<std::size_t>(maxPuncPerSeed_),
  ...
  const int maxPunctureCount =
      std::min(std::max(0, options_.npMax), maxPuncPerSeed_);
  if (maxStateCount <= 0)
  {
    endRegion = 98;
    return TraceStatus::InvalidConfiguration;
  ...
  trajectories[trajBase + static_cast<std::size_t>(localTrajCount)] =
      modelExec.reconstructTrajectoryXYZ(initial);
  ++localStateCount;
  ++localTrajCount;
  ...
  while (region < 10 && localStateCount < maxStateCount)
  {
    for (int iy = 0; iy < d.ny - 1; ++iy)
    {
      ...
      if (region == 0 && yStart > d.nypf1 && yStart < d.nypf2 + 1)
      {
        ...
        trajectories[trajBase + static_cast<std::size_t>(localTrajCount)] =
            modelExec.reconstructTrajectoryXYZ(step);
        ++localStateCount;
        ++localTrajCount;
        ...
        if (maxPunctureCount > 0)
        {
          tryDetectPunctureOnLastSegment(
              modelExec, states, trajectories, stateBase, trajBase,
              localStateCount, options_.direction, maxPunctureCount, punctures,
              punctureValid, punctureBase, localPunctureCount, lastFitRoot);
        }
      ...
      else if (region == 1 || region == 2)
      {
        ...
        if (options_.direction == 1 && yStart == d.nypf1 && region == 2)
          yEnd = d.nypf2 + 1;
        else if (options_.direction == -1 && yStart == d.nypf2 + 1 &&
                 region == 2)
        {
          yEnd = d.nypf1;
        }
        ...
        {
          xind = modelExec.interp1(d.xarray, d.xiarray, next.x);
          if (xind < static_cast<double>(d.ixsep) + 0.5 && yEnd > d.nypf1 &&
              yEnd < d.nypf2 + 1)
          {
            region = 0;
          }
  ...
  {
    const Point3D &p1 =
        trajectories[trajBase + static_cast<std::size_t>(i - 1)];
...
TraceStatus FieldLineIntegrator::traceLine(const Point3D &seedInd,
                                           LineTraceResult &out) const
{
  std::vector<TrajectoryState> states(
      static_cast<std::size_t>(maxStatesPerSeed_));
  ...
  std::vector<std::uint8_t> punctureValid(
      static_cast<std::size_t>(maxPuncPerSeed_), static_cast<std::uint8_t>(0));
  ...
  const TraceStatus status = traceLine(
      seedInd, 0,
      makeTraceOutputViews(states, trajectories, punctures, &punctureValid),
      stateCount, trajCount, punctureCount, endRegion, connectionLength, iline);
  ...
  const int clampedStateCount =
      std::max(0, std::min(stateCount, maxStatesPerSeed_));
  const int clampedTrajCount =
      std::max(0, std::min(trajCount, maxTrajPerSeed_));
  const int clampedPunctureCount =
      std::max(0, std::min(punctureCount, maxPuncPerSeed_));

  out.states.assign(states.begin(), states.begin() + clampedStateCount);
  out.trajectoryXYZ.assign(trajectories.begin(),
                           trajectories.begin() + clampedTrajCount);
  out.punctures.assign(punctures.begin(),
                       punctures.begin() + clampedPunctureCount);
  return status;
}
