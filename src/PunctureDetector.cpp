...
CrossingEval evaluateCrossing(const AparFieldModel &model,
                              const LineTraceResult &line, int tc0, int tc1,
...
void PunctureDetector::detect(LineTraceResult &line, int direction,
                              int npMax) const
{
  ...
  for (size_t seg = 0;
       seg + 1 < n && static_cast<int>(line.punctures.size()) < npMax; ++seg)
  {
    const std::vector<double> roots = Interpolator::solveCubicSegmentRoots(
        spline.values()[seg], spline.b()[seg], spline.c()[seg],
        spline.d()[seg]);
    ...
    for (double alpha : localRoots)
    {
      ...
      CrossingEval crossing =
          evaluateCrossing(model_, line, tc0, tc1, direction, alpha);
