...
double value3D(const std::vector<double> &data, int ny, int nz, int ix, int iy,
               int iz)
{
...
void NaturalCubicSpline::build(const std::vector<double> &x,
                               const std::vector<double> &y)
{
  ...
  for (int i = 1; i < nm1; ++i)
  {
    if (std::fabs(h[i]) < 1.0e-20 || std::fabs(h[i - 1]) < 1.0e-20)
      continue;
    alpha[i] =
        3.0 / h[i] * (y_[i + 1] - y_[i]) - 3.0 / h[i - 1] * (y_[i] - y_[i - 1]);
  }
  ...
  for (int j = nm1 - 1; j >= 0; --j)
  {
    ...
    {
      b_[j] =
          (y_[j + 1] - y_[j]) / h[j] - h[j] * (c_[j + 1] + 2.0 * c_[j]) / 3.0;
...
{
  return y_[segment] + b_[segment] * t + c_[segment] * t * t +
         d_[segment] * t * t * t;
}
...
double Interpolator::linear1DStride(const std::vector<double> &xp,
                                    const double *fp, int n, int stride,
                                    double x)
{
  if (n <= 0 || fp == nullptr)
    return 0.0;
                               ...
                               int nz, double x, double y, double z)
{
  ...
  int ix0 = (x <= xcoords.front())
                ? 0
                : ((x >= xcoords.back()) ? nx - 2 : lowerBracket(xcoords, x));
  int iy0 = (y <= ycoords.front())
                ? 0
                : ((y >= ycoords.back()) ? ny - 2 : lowerBracket(ycoords, y));
  int iz0 = (z <= zcoords.front())
                ? 0
                : ((z >= zcoords.back()) ? nz - 2 : lowerBracket(zcoords, z));
...
double Interpolator::lagrange4(double x, double x0, double x1, double x2,
                               double x3, double y0, double y1, double y2,
                               double y3)
{
...
std::vector<double> Interpolator::solveCubicSegmentRoots(double a0, double a1,
                                                         double a2, double a3)
{
  ...
  {
    const double phi = std::acos(std::max(
        -1.0, std::min(1.0, (-0.5 * q) / std::sqrt(-(p * p * p) / 27.0))));
