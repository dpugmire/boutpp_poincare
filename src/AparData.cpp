...
{
  int dimid = -1;
  ncCheck(nc_inq_dimid(ncid, name.c_str(), &dimid),
          "nc_inq_dimid(" + name + ")");
  size_t dimLen = 0;
...
std::vector<double> readVarDouble(int ncid, const std::string &name)
{
  ...
  ncCheck(nc_inq_varndims(ncid, varid, &ndims),
          "nc_inq_varndims(" + name + ")");

  std::vector<int> dimids(static_cast<size_t>(ndims), 0);
  ncCheck(nc_inq_vardimid(ncid, varid, dimids.data()),
          "nc_inq_vardimid(" + name + ")");
  ...
  ncCheck(nc_get_var_double(ncid, varid, data.data()),
...
std::vector<std::string> getVarDimNames(int ncid, const std::string &name)
{
  ...
  for (int dimid : dimids)
  {
    ...
    ncCheck(nc_inq_dimname(ncid, dimid, dimName),
            "nc_inq_dimname(" + name + ")");
...
void setIxIyIzValue(std::vector<double> &out, int ny, int nzG, int ix, int iy,
                    int iz, double value)
{
  out[(static_cast<size_t>(ix) * ny + iy) * nzG + iz] = value;
}

void setIxIzValue(std::vector<double> &out, int nzG, int ix, int iz,
                  double value)
{
  out[static_cast<size_t>(ix) * nzG + iz] = value;
}

std::vector<double> convert2DToIxIy(int ncid, const std::string &varName,
                                    int nx, int ny)
{
  ...
  if (dims[0] == "ny" && dims[1] == "nx")
  {
    for (int iy = 0; iy < ny; ++iy)
    {
      ...
      {
        out[static_cast<size_t>(ix) * ny + iy] =
            raw[static_cast<size_t>(iy) * nx + ix];
      }
    }
    return out;
  }
  ...
  if (dims[0] == "nx" && dims[1] == "ny")
  {
    for (int ix = 0; ix < nx; ++ix)
    {
      ...
      {
        out[static_cast<size_t>(ix) * ny + iy] =
            raw[static_cast<size_t>(ix) * ny + iy];
      }
    }
    return out;
  }
...
std::vector<double> convert2DToIxIyLocal(int ncid, const std::string &varName,
                                         int nx, int ny,
  ...
  if (dims[0] == "nz" && dims[1] == "ny" && dims[2] == "nx")
  {
    for (int iz = 0; iz < nz; ++iz)
    {
      for (int iy = 0; iy < ny; ++iy)
      {
        ...
        {
          const double value =
              raw[(static_cast<size_t>(iz) * ny + iy) * nx + ix];
  ...
  if (dims[0] == "nx" && dims[1] == "ny" && dims[2] == "nz")
  {
    for (int ix = 0; ix < nx; ++ix)
    {
      for (int iy = 0; iy < ny; ++iy)
      {
        ...
        {
          const double value =
              raw[(static_cast<size_t>(ix) * ny + iy) * nz + iz];
          for (int zp = 0; zp < zperiod; ++zp)
            setIxIyIzValue(out, ny, nzG, ix, iy, zp * nz + iz, value);
        }
      }
...
std::vector<double> convert2DXZReplicated(int ncid, const std::string &varName,
                                          int nx, int nz, int zperiod)
{
  const std::vector<std::string> dims = getVarDimNames(ncid, varName);
  if (dims.size() != 2)
    throw std::runtime_error(varName + " must be 2D");
...
{
  int ncid = -1;
  ncCheck(nc_open(aparPath.c_str(), NC_NOWRITE, &ncid),
          "nc_open(" + aparPath + ")");
  ...
  try
  {
    ...
    rxy_cfr = convert2DToIxIyLocal(ncid, "rxy_cfr", nx_cfr, ny_cfr, "nx_cfr",
                                   "ny_cfr");
    zxy_cfr = convert2DToIxIyLocal(ncid, "zxy_cfr", nx_cfr, ny_cfr, "nx_cfr",
                                   "ny_cfr");
    zShift_cfr = convert2DToIxIyLocal(ncid, "zShift_cfr", nx_cfr, ny_cfr,
                                      "nx_cfr", "ny_cfr");
  }
  catch (...)
  {
