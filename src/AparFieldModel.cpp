...
int wrapZIndex(const AparData &data, int izExt)
{
  if (data.nzG <= 0)
    return 0;

  int wrapped = izExt % data.nzG;
  if (wrapped < 0)
    wrapped += data.nzG;
  return wrapped;
}

void periodicZStencilCentered(int baseIdx, int outIdx[4])
{
  outIdx[0] = baseIdx - 1;
  outIdx[1] = baseIdx;
  outIdx[2] = baseIdx + 1;
...
double samplePeriodicRow3D(const AparData &data,
                           const std::vector<double> &data3d, int ix, int iy,
                           int izExt)
{
  return data3d[data.idx3(ix, iy, wrapZIndex(data, izExt))];
}

double samplePeriodicXZ(const AparData &data, const std::vector<double> &data2d,
                        int ix, int izExt)
{
  return data2d[data.idx_xz(ix, wrapZIndex(data, izExt))];
}
...
double
AparFieldModel::interpPeriodicRow3DSpline(const std::vector<double> &data3d,
                                          int ix, int iy, double z) const
{
  ix = Interpolator::clampInt(ix, 0, data_.nx - 1);
  iy = Interpolator::clampInt(iy, 0, data_.ny - 1);
...
double AparFieldModel::interpXZ3DAtY(const std::vector<double> &data3d, int y0,
...
double AparFieldModel::interpXZ3DAtYSpline(const std::vector<double> &data3d,
                                           int y0, double x, double z) const
{
  ...
  const double xvals[4] = {data_.xarray[ixs[0]], data_.xarray[ixs[1]],
                           data_.xarray[ixs[2]], data_.xarray[ixs[3]]};
  ...
  for (int i = 0; i < 4; ++i)
  {
    ...
    const double zv0 = samplePeriodicRow3D(data_, data3d, ix, y0, izs[0]);
    const double zv1 = samplePeriodicRow3D(data_, data3d, ix, y0, izs[1]);
    const double zv2 = samplePeriodicRow3D(data_, data3d, ix, y0, izs[2]);
    const double zv3 = samplePeriodicRow3D(data_, data3d, ix, y0, izs[3]);

    zinterp[i] =
        Interpolator::lagrange4(zq, zc0, zc1, zc2, zc3, zv0, zv1, zv2, zv3);
  }

  return Interpolator::lagrange4(xq, xvals[0], xvals[1], xvals[2], xvals[3],
                                 zinterp[0], zinterp[1], zinterp[2],
                                 zinterp[3]);
}

double AparFieldModel::interpXZ2D(const std::vector<double> &data2d, double x,
                                  double z) const
{
  if (data_.nx < 2 || data_.nzG < 1)
    return 0.0;
                                        ...
                                        double x, double z) const
{
  ...
  for (int i = 0; i < 4; ++i)
  {
    ...
    const double zv0 = samplePeriodicXZ(data_, data2d, ix, izs[0]);
    const double zv1 = samplePeriodicXZ(data_, data2d, ix, izs[1]);
    const double zv2 = samplePeriodicXZ(data_, data2d, ix, izs[2]);
    const double zv3 = samplePeriodicXZ(data_, data2d, ix, izs[3]);
...
void AparFieldModel::evaluateStage(const XZPoint &point, int yStart1b,
...
Point3D
AparFieldModel::reconstructTrajectoryXYZ(const TrajectoryState &state) const
{
                                               ...
                                               double zvalue) const
{
  ...
  {
    rxyValue = Interpolator::spline2D(data_.xiarray_cfr, data_.yiarray_cfr,
                                      data_.rxy_cfr, data_.nx_cfr, data_.ny_cfr,
                                      ind.x, ind.y);
    zxyValue = Interpolator::spline2D(data_.xiarray_cfr, data_.yiarray_cfr,
                                      data_.zxy_cfr, data_.nx_cfr, data_.ny_cfr,
                                      ind.x, ind.y);
    zsValue = Interpolator::spline2D(data_.xiarray_cfr, data_.yiarray_cfr,
                                     data_.zShift_cfr, data_.nx_cfr,
                                     data_.ny_cfr, ind.x, ind.y);
  }
  else
  {
    rxyValue = Interpolator::spline2D(data_.xiarray, data_.yiarray, data_.rxy,
                                      data_.nx, data_.ny, ind.x, ind.y);
    zxyValue = Interpolator::spline2D(data_.xiarray, data_.yiarray, data_.zxy,
                                      data_.nx, data_.ny, ind.x, ind.y);
    zsValue = Interpolator::spline2D(data_.xiarray, data_.yiarray, data_.zShift,
                                     data_.nx, data_.ny, ind.x, ind.y);
  }
