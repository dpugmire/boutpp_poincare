...
// Read scalar
int NetCDFLoader::readScalar(const std::string& varName)
{
  int varid;
  ...
  int value;
  status = nc_get_var_int(this->ncid, varid, &value);
  if (status != NC_NOERR)
    throw std::runtime_error("Error reading scalar variable: " + std::string(nc_strerror(status)));

  return value;
}

// Read 1D variable
std::vector<double> NetCDFLoader::read1DVariable(const std::string& varName)
{
  int varid;
  int status = nc_inq_varid(this->ncid, varName.c_str(), &varid);
  if (status != NC_NOERR)
...
// Read 2D variable
std::vector<std::vector<double>> NetCDFLoader::read2DVariable(const std::string& varName, bool transpose)
{
  int varid;
  nc_type varType;
...
// Read 3D variable
std::vector<std::vector<std::vector<double>>> NetCDFLoader::read3DVariable(const std::string& varName, bool permute)
{
  int varid;
  nc_type varType;
