...
std::tuple<std::vector<double>, std::vector<size_t>, std::vector<double>> find_zero_crossings(const std::vector<double>& itarray,
...
void dumpTrajSamples(int iline, std::vector<std::vector<double>> points)
{
  int n = points.size();
...
void processPunctures(const std::vector<double>& itarray,
                      const std::vector<double>& fl_x3d,
...
*/
  ...
  for (const auto& iline : LINES)
  {
    ...
    for (int i = 0; i < nc; i++)
    {
      ...
      {
        zvalue = b * INTERP(opts.ziarray, opts.zarray, pt.traj4) + a * INTERP(opts.ziarray, opts.zarray, pt_1.traj4);
