...
public:
  ...
  using DoublePortal = typename DoubleArray::ReadPortalType;

  ViskoresAparFieldExecutionObject(const DoublePortal& xiarrayIn,
                                   const DoublePortal& xarrayIn,
                                   const DoublePortal& ziarrayIn,
                                   const DoublePortal& zarrayIn,
                                   const DoublePortal& dxdyIn,
                                   const DoublePortal& dzdyIn,
                                   const DoublePortal& dxdyP1In,
                                   const DoublePortal& dzdyP1In,
                                   const DoublePortal& dxdyM1In,
                                   const DoublePortal& dzdyM1In,
                                   const DoublePortal& shiftAngleIn,
                                   int nxIn,
                                   int nyIn,
                                   int nzGIn,
                                   int ixsepIn,
  ...
  VISKORES_EXEC int lowerBracket(const PortalType& xp, double x) const
  {
  ...
  VISKORES_EXEC double interp1(const XPortalType& xp, const FPortalType& fp, double x) const
  {
    const int n = static_cast<int>(xp.GetNumberOfValues());
    ...
    const int i1 = i0 + 1;
    const double denom = xp.Get(i1) - xp.Get(i0);
    if (std::fabs(denom) < 1.0e-20)
    {
      return fp.Get(i0);
    }
    const double t = (x - xp.Get(i0)) / denom;
    return fp.Get(i0) + t * (fp.Get(i1) - fp.Get(i0));
  }

  VISKORES_EXEC double wrapZ(double z) const
  {
    if (zmax <= 0.0)
    {
      return z;
    }
    double out = std::fmod(z, zmax);
    if (out < 0.0)
    {
      out += zmax;
    }
  ...
  VISKORES_EXEC viskores::Id idx3(int ix, int iy, int iz) const
  {
  ...
  VISKORES_EXEC viskores::Id idxXZ(int ix, int iz) const
  {
    return static_cast<viskores::Id>(ix) * static_cast<viskores::Id>(nzG) + static_cast<viskores::Id>(iz);
  }

  VISKORES_EXEC void cubicStencilCentered(int baseIdx, int nNodes, int outIdx[4]) const
  {
    if (nNodes < 4)
    {
      outIdx[0] = 0;
  ...
  VISKORES_EXEC double lagrange4(double x,
                                 double x0,
                                 double x1,
                                 double x2,
                                 double x3,
  ...
  VISKORES_EXEC double sampleClampedRow3D(const DoublePortal& data3d, int ix, int iy, int izExt) const
  {
    if (izExt <= 0)
    {
  ...
  VISKORES_EXEC double sampleClampedXZ(const DoublePortal& data2d, int ix, int izExt) const
  {
    if (izExt <= 0)
    {
  ...
  VISKORES_EXEC double interpXZ3DAtY(const DoublePortal& data3d, int y0, double x, double z) const
  {
    if (nx < 2 || nzG < 1)
    {
      return 0.0;
    ...
    else
    {
      ...
      const double denom = xarray.Get(ix0 + 1) - xarray.Get(ix0);
    ...
    const double zWrapped = wrapZ(z);
    const double zf = zWrapped / dz_torus;
    ...
    const double v00 = data3d.Get(idx3(ix0, y0, iz0));
    const double v01 = data3d.Get(idx3(ix0, y0, iz1));
    const double v10 = data3d.Get(idx3(ix1, y0, iz0));
    const double v11 = data3d.Get(idx3(ix1, y0, iz1));
  ...
  VISKORES_EXEC double interpXZ3DAtYSpline(const DoublePortal& data3d, int y0, double x, double z) const
  {
    if (nx < 4 || nzG < 4)
    {
    ...
    const double xvals[4] = {
      xarray.Get(ixs[0]), xarray.Get(ixs[1]), xarray.Get(ixs[2]), xarray.Get(ixs[3])
    };
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      const double zc0 = static_cast<double>(izs[0]) * dz_torus;
      const double zc1 = static_cast<double>(izs[1]) * dz_torus;
      const double zc2 = static_cast<double>(izs[2]) * dz_torus;
      const double zc3 = static_cast<double>(izs[3]) * dz_torus;

      const double zv0 = sampleClampedRow3D(data3d, ix, y0, izs[0]);
      const double zv1 = sampleClampedRow3D(data3d, ix, y0, izs[1]);
      const double zv2 = sampleClampedRow3D(data3d, ix, y0, izs[2]);
      const double zv3 = sampleClampedRow3D(data3d, ix, y0, izs[3]);
  ...
  VISKORES_EXEC double interpXZ2D(const DoublePortal& data2d, double x, double z) const
  {
    if (nx < 2 || nzG < 1)
    {
      return 0.0;
    ...
    const double v00 = data2d.Get(idxXZ(ix0, iz0));
    const double v01 = data2d.Get(idxXZ(ix0, iz1));
    const double v10 = data2d.Get(idxXZ(ix1, iz0));
    const double v11 = data2d.Get(idxXZ(ix1, iz1));
  ...
  VISKORES_EXEC double interpXZ2DSpline(const DoublePortal& data2d, double x, double z) const
  {
    if (nx < 4 || nzG < 4)
    {
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      const double zv0 = sampleClampedXZ(data2d, ix, izs[0]);
      const double zv1 = sampleClampedXZ(data2d, ix, izs[1]);
      const double zv2 = sampleClampedXZ(data2d, ix, izs[2]);
      const double zv3 = sampleClampedXZ(data2d, ix, izs[3]);
  ...
  VISKORES_EXEC void evaluateStage(const XZPoint& point, int yStart1b, int region, int direction, int stage, XZDeriv& deriv) const
  {
    ...
    const double dxP = interpXZ3DAtYSpline(dxdy, yp, point.x, point.z);
    const double dzP = interpXZ3DAtYSpline(dzdy, yp, point.x, point.z);
  ...
  DoublePortal xiarray;
  DoublePortal xarray;
  DoublePortal ziarray;
  DoublePortal zarray;
  DoublePortal dxdy;
  DoublePortal dzdy;
  DoublePortal dxdy_p1;
  DoublePortal dzdy_p1;
  DoublePortal dxdy_m1;
  DoublePortal dzdy_m1;
  DoublePortal shiftAngle;
...
public:
  ...
  explicit ViskoresAparField(const AparData& data)
    : xiarray_(viskores::cont::make_ArrayHandle(data.xiarray, viskores::CopyFlag::On))
    , xarray_(viskores::cont::make_ArrayHandle(data.xarray, viskores::CopyFlag::On))
    , ziarray_(viskores::cont::make_ArrayHandle(data.ziarray, viskores::CopyFlag::On))
    , zarray_(viskores::cont::make_ArrayHandle(data.zarray, viskores::CopyFlag::On))
    , dxdy_(viskores::cont::make_ArrayHandle(data.dxdy, viskores::CopyFlag::On))
    , dzdy_(viskores::cont::make_ArrayHandle(data.dzdy, viskores::CopyFlag::On))
    , dxdyP1_(viskores::cont::make_ArrayHandle(data.dxdy_p1, viskores::CopyFlag::On))
    , dzdyP1_(viskores::cont::make_ArrayHandle(data.dzdy_p1, viskores::CopyFlag::On))
    , dxdyM1_(viskores::cont::make_ArrayHandle(data.dxdy_m1, viskores::CopyFlag::On))
    , dzdyM1_(viskores::cont::make_ArrayHandle(data.dzdy_m1, viskores::CopyFlag::On))
    , shiftAngle_(viskores::cont::make_ArrayHandle(data.shiftAngle, viskores::CopyFlag::On))
    , nx_(data.nx)
    , ny_(data.ny)
    , nzG_(data.nzG)
    , ixsep_(data.ixsep)
...
private:
  DoubleArray xiarray_;
  DoubleArray xarray_;
  DoubleArray ziarray_;
  DoubleArray zarray_;
  DoubleArray dxdy_;
  DoubleArray dzdy_;
  DoubleArray dxdyP1_;
...
{
public:
  ViskoresTraceStatesWorklet(int maxStatesPerSeed, int direction)
    : maxStatesPerSeed_(maxStatesPerSeed)
    , direction_(direction)
  {
  }

  using ControlSignature = void(FieldIn seeds,
                                ExecObject field,
                                ...
                                StatePortalType& statesOut) const
  {
    ...
    statusCodes.Set(idx, static_cast<int>(TraceStatus::Ok));

    if (maxStatesPerSeed_ <= 0)
    {
      statusCodes.Set(idx, static_cast<int>(TraceStatus::InvalidConfiguration));
      endRegions.Set(idx, 98);
      return;
    }

    const double xindSeed = seedInd.x;
    if (xindSeed < 1.0 || xindSeed > static_cast<double>(field.nx))
    {
      statusCodes.Set(idx, static_cast<int>(TraceStatus::InvalidSeed));
    ...
    int region = 1;
    if (xind < static_cast<double>(field.ixsep) + 0.5)
    {
      region = 0;
    ...
    while (region < 10 && localStateCount < maxStatesPerSeed_)
    {
      for (int iy = 0; iy < field.ny - 1; ++iy)
      {
        ...
        if (region == 0 && yStart > field.nypf1 && yStart < field.nypf2 + 1)
        {
          ...
          {
            const double shift = field.interp1(field.xiarray, field.shiftAngle, xind);
            next.z += shift;
            yEnd = field.nypf1 + 1;
          }
          ...
          const double zind = field.interp1(field.zarray, field.ziarray, next.z);
        ...
        else if (region == 1 || region == 2)
        {
          ...
          {
            xind = field.interp1(field.xarray, field.xiarray, next.x);
            if (xind < static_cast<double>(field.ixsep) + 0.5 && yEnd > field.nypf1 && yEnd < field.nypf2 + 1)
            {
              region = 0;
            }
            else if (xind < static_cast<double>(field.ixsep) + 0.5 && (yEnd > field.nypf2 - 1 || yEnd < field.nypf1))
            {
              region = 2;
            }
          }
    ...
    if (region < 10)
    {
      statusCodes.Set(idx, static_cast<int>(TraceStatus::MaxStepLimitReached));
    }
  }

private:
  template <typename FieldExecType>
  VISKORES_EXEC void rk4Step(const FieldExecType& field,
                             const XZPoint& start,
                             int yStart,
                             int region,
                             int direction,
